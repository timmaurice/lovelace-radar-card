...
| `radar_max_distance`          | number  | `100`                                             | The maximum distance shown on the radar (in km or mi). Ignored if `auto_radar_max_distance` is `true`. Entities beyond it are drawn hollow on the outer ring and marked in the legend. |
| `enable_markers`              | boolean | `false`                                           | Enables adding and displaying persistent markers. A new marker is placed at the radar's center, so markers are most useful with a `center_entity` (moving mode).                       |
...
#### Entities the radar cannot plot

An entity the card cannot place is never silently dropped. It is listed under the radar with the reason:

- **not found** — no entity with that id exists (usually a typo, or a renamed entity).
- **no location** — the entity exists but has no `latitude`/`longitude` attributes.
- **unavailable** — the entity is `unavailable` or `unknown`. Its last known coordinates are deliberately not
  plotted, because a stale position drawn as a live one is worse than no position at all.

Entities hidden on purpose by `hide_at_home` are not listed — that is what was asked for.

With a fixed `radar_max_distance` (that is, `auto_radar_max_distance: false`), an entity further away than the
radar reaches is drawn as a hollow dot on the outer ring rather than off the edge of the chart, and both its
legend entry and its tooltip say that it is beyond the radar's range.
