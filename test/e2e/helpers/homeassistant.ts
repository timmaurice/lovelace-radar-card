...
export const BUNDLE_NAME = 'radar-card.js';
...
/**
 * The bundle is committed and bind-mounted into the container read-only, so the
 * browser gets whatever is on disk - not what `src/` currently says. Rollup
 * stamps `v<version>` into the banner it prints on load, so the version in the
 * file is a cheap way of noticing that the committed bundle predates the
 ...
 *
 * Lovelace resources and the template entities `configuration.yaml` defines
 * land well after the HTTP server starts serving. A check that stops at the
 * frontend can therefore read a half-set-up instance and fail on things that
 * were only late, not broken.
 */
